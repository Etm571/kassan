...
export async function PUT(req: NextRequest) {
  ...
  if (!id || !name || !barcode) {
    return NextResponse.json({ error: 'ID, namn och streckkod krävs' }, {
      status: 400,
      headers: { 'Access-Control-Allow-Origin': '*' }
    })
  }
  ...
  if (price !== undefined) {
    ...
    if (
      ...
      return NextResponse.json({ error: 'max två decimaler' }, {
        status: 400,
        headers: { 'Access-Control-Allow-Origin': '*' }
      })
    }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Kunde inte uppdatera' }, {
      status: 500,
      headers: { 'Access-Control-Allow-Origin': '*' }
    })
  }
